...
function ChannelForm({ userId, onChannelAdded }) {
  ...
  const isValidTelegramChannel = (value) => {
    return /^@[\w\d_]{5,}$/.test(value) || /^https?:\/\/t\.me\/[\w\d_]{5,}$/.test(value)
  }

  const handleSubmit = async (e) => {
    e.preventDefault()
    if (!isValidTelegramChannel(channelId)) {
      setError('Введите корректный Telegram канал, например @mychannel или https://t.me/mychannel')
      return
    }
    setError('')
    ...
    try {
      ...
      const res = await fetch(`http://77.105.133.23:8087/api/channels/${userId}`, {
        method: 'POST',
        headers: { 
